...
public class LikeService {
    ...
    public ResponseEntity<?> postLikes(Long userId, Long tableId) {
        ...
        Dish dish = tablesRepository.findById(tableId).orElseThrow(() -> new IllegalArgumentException("술상게시물이 없습니다"));

        Optional<Likes> Likefound = likesRepository.findByUserAndDish(user, dish);
        ...
        Likes like = new Likes(user, dish);
    ...
    public ResponseEntity<?> deleteLikes(User user, Long tableId) {
        ...
        for (Likes like : likes) {
            if (like.getDish().getId().equals(findTable.getId())) {
